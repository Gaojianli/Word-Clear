...
public ref class wordSchemaWithCommitter:public wordSchema {
public:
	wordSchemaWithCommitter(String^ word, int level, String^ committerName) :wordSchema(word, level), committerName(committerName) {};
...
ref class CompareWord :System::Collections::Generic::Comparer<wordSchemaWithCommitter^> {
public:
	virtual int Compare(wordSchemaWithCommitter^ a, wordSchemaWithCommitter^ b) override {
		return a->level < b->level;//count larger come first
	}
};
