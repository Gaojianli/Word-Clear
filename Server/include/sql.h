...
public:
	...
	static std::vector<WordWithCommitter> *fetchQestionWithName();
	template <typename T>
...
inline std::vector<User> *sql::fetchUsersByCondition(std::string properties, const T &value)
{
	...
	if constexpr (std::is_same_v<std::decay_t<T>, string> || std::is_same_v<std::decay_t<T>, const char *>)
		sqlCommand.append("'%%");
	if constexpr (std::is_same_v<std::decay_t<T>, bool>)
		sqlCommand += value ? "true" : "false";
	else if constexpr (std::is_same_v<std::decay_t<T>, string> || std::is_same_v<std::decay_t<T>, const char *>)
