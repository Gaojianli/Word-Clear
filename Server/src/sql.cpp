...
sql::sql()
{
	con.open({"localhost", "[USER_NAME]", "[USER_PASSWORD]", "word_clear_game", 0, true});
	if (!con)
	{
...
std::vector<WordWithCommitter> *sql::fetchQestionWithName()
{
	std::vector<WordWithCommitter> *toReturn = nullptr;
	_instance->con.query("SELECT word,question.level,committer,name from question inner join user on question.committer=user.id")
		.each([&toReturn](std::string word, int level, int committer, std::string name) {
			if (toReturn == nullptr)
				toReturn = new std::vector<WordWithCommitter>();
			toReturn->push_back(WordWithCommitter(word.c_str(), level, committer, name));
			return true;
		});
	return toReturn;
}